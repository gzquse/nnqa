#!/usr/bin/env python3
"""
IonQ Native Polynomial Results Plotter
=======================================

Generates publication-quality figures from IonQ native polynomial experiments.

Usage:
    python plot_ionq_results.py --input results
...
def load_ionq_results(input_dir):
    """Load all H5 result files from IonQ experiments."""
    ...
    for h5_file in sorted(h5_files):
        try:
            ...
            result = {
                ...
                'backend': meta.get('backend', 'simulator'),
    ...
    print(f"Loaded {len(all_data)} IonQ result files from {input_dir}")
    return all_data
...
def create_figure1_degree_scaling(aggregated, output_path):
    """Create Figure 1: RMSE vs polynomial degree with error bars (IonQ)."""
    ...
    ax1.bar(x_pos + width/2, q_rmse_mean, width, yerr=q_rmse_std,
           color=colors['ionq'], label='IonQ Quantum',
           edgecolor='black', linewidth=0.5, capsize=3)
    
    ax1.set_xlabel('Polynomial Degree')
    ax1.set_ylabel('RMSE')
    ax1.set_title('(a) Recovery Error vs Polynomial Degree\n(IonQ Native Arithmetic)')
    ...
    ax2.errorbar(x_pos + 0.1, q_corr_mean, yerr=q_corr_std,
                ...
                capsize=4, capthick=1.5, label='IonQ Quantum',
                markeredgecolor='black', markeredgewidth=0.5)
...
def create_figure2_recovery_grid(aggregated, output_path):
    """Create Figure 2: 2x3 grid showing recovery for each degree (IonQ)."""
    configure_publication_style()
...
def create_figure3_error_distribution(aggregated, output_path):
    """Create Figure 3: Error distribution histograms by degree (IonQ)."""
    configure_publication_style()
    ...
    for idx, degree in enumerate(degrees):
        ...
        # Histogram
        bins = np.linspace(-0.15, 0.15, 35)
        
        ax.hist(all_classical_errors, bins=bins, color=colors['classical'],
               alpha=0.5, label='Classical', edgecolor='black', linewidth=0.3)
        ax.hist(all_quantum_errors, bins=bins, color=colors['ionq'],
               alpha=0.5, label='IonQ Quantum', edgecolor='black', linewidth=0.3)
...
def create_figure4_correlation_summary(aggregated, all_data, output_path):
    """Create Figure 4: Correlation analysis scatter plots (IonQ)."""
    configure_publication_style()
    ...
    ax1.set_ylabel('IonQ Measured Value')
    ...
    bars = ax2.bar(degrees, pass_rates, color=colors['ionq'], alpha=0.7,
                  edgecolor='black', linewidth=0.5, label='Pass Rate')
    ax2.set_ylabel('Pass Rate (%)', color=colors['ionq'])
    ax2.set_ylim(0, 110)
    ax2.tick_params(axis='y', labelcolor=colors['ionq'])
...
def generate_latex_table(aggregated, output_path):
    """Generate LaTeX table of results for IonQ approach."""
    degrees = sorted(aggregated.keys())
    
    latex = r"""
\begin{table}[ht]
\centering
\caption{Polynomial Recovery Accuracy: IonQ Native Quantum Arithmetic}
\label{tab:results-ionq}
...
def print_summary_table(aggregated):
    ...
    print("\n" + "=" * 85)
    print("IONQ NATIVE POLYNOMIAL RESULTS SUMMARY")
    print("=" * 85)
    print(f"{'Deg':<5} {'Name':<12} {'Q-RMSE':<15} {'C-RMSE':<15} {'Corr':<10} {'Pass%':<10}")
    print("-" * 85)
    
    for degree in degrees:
        data = aggregated[degree]
        q_rmse = f"{data['quantum_rmse_mean']:.4f}+/-{data['quantum_rmse_std']:.4f}"
        c_rmse = f"{data['classical_rmse_mean']:.4f}+/-{data['classical_rmse_std']:.4f}"
        corr = f"{data['quantum_corr_mean']:.4f}"
        pass_rate = f"{data['pass_rate_mean']*100:.1f}%"
        
        print(f"{degree:<5} {data['name']:<12} {q_rmse:<15} {c_rmse:<15} {corr:<10} {pass_rate:<10}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Generate publication figures for IonQ native polynomial results'
    )
    ...
    parser.add_argument('--provider', default=None,
                       help='Filter by provider (e.g., ionq, qiskit_aer)')
...
if __name__ == '__main__':
    ...
    # Load data
    all_data = load_ionq_results(input_dir)
    
    # Filter by provider if requested
    if args.provider:
        all_data = [d for d in all_data if d.get('provider') == args.provider]
        print(f"Filtered to {len(all_data)} results from provider: {args.provider}")
    
    if not all_data:
        print("No IonQ result data found!")
        print(f"Expected H5 files in: {input_dir}")
        print("\nRun experiments first:")
        print("  python submit_ionq_native.py --backend simulator --execute")
        sys.exit(1)
    ...
    print("\n" + "=" * 60)
    print("GENERATING IONQ NATIVE POLYNOMIAL FIGURES")
    print("=" * 60)
