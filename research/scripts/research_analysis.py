#!/usr/bin/env python3
"""
Research Analysis: Publication-Quality Figures for NN-to-Quantum Study
=======================================================================

Generates 4 main figures and LaTeX tables from experimental results.

Usage:
    python research_analysis.py --input results/local --output figures/local
    python research_analysis.py --all
"""
...
def load_all_results(input_dir):
    ...
    print(f"Loaded {len(all_data)} result files from {input_dir}")
    return all_data
...
def create_figure1_degree_scaling(aggregated, output_path):
    ...
    ax1.bar(x_pos + width/2, q_rmse_mean, width, yerr=q_rmse_std,
           color=colors['quantum'], label='Quantum Circuit',
           edgecolor='black', linewidth=0.5, capsize=3)
    ...
    ax1.set_ylim(0, max(q_rmse_mean) * 1.5)
    ...
    ax2.errorbar(x_pos + 0.1, q_corr_mean, yerr=q_corr_std,
                ...
                capsize=4, capthick=1.5, label='Quantum Circuit',
                markeredgecolor='black', markeredgewidth=0.5)
...
def create_figure2_recovery_grid(aggregated, output_path):
    """Create Figure 2: 2x3 grid showing recovery for each degree."""
    configure_publication_style()
...
def create_figure3_error_distribution(aggregated, output_path):
    """Create Figure 3: Error distribution histograms by degree."""
    configure_publication_style()
    ...
    for idx, degree in enumerate(degrees):
        ...
        ax.hist(all_quantum_errors, bins=bins, color=colors['quantum'],
               alpha=0.5, label='Quantum', edgecolor='black', linewidth=0.3)
...
def create_figure4_correlation_summary(aggregated, all_data, output_path):
    """Create Figure 4: Correlation analysis scatter plots."""
    configure_publication_style()
    ...
    ax1.set_ylabel('Quantum Measured Value')
...
\begin{table}[ht]
\centering
\caption{Polynomial Recovery Accuracy: Classical NN vs Quantum Circuit}
\label{tab:results}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Generate publication figures from research results'
    )
    parser.add_argument('--input', default='results/local',
                       help='Input directory with H5 result files')
    parser.add_argument('--output', default='figures/local',
